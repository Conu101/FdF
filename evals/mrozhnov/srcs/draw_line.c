...
/*   Updated: 2022/05/18 19:46:20 by mrozhnova        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/fdf.h"

static void	draw_pixel(int x, int y, int color, t_mlx *mlx)
{
	int	r;
	int	g;
	int	b;
...
static void	drawline_low(t_line *line, t_mlx *mlx)
{
	int	*dxy;
	int	*xy;
	int	yi;
...
static void	drawline_high(t_line *line, t_mlx *mlx)
{
	int	*dxy;
	int	*xy;
	int	xi;
...
static void	swap_points(t_line *line)
{
	int	tmp;
...
void	bresenham(t_line *line, t_mlx *mlx)
{
	if (abs(line->y2 - line->y1) < abs(line->x2 - line->x1))
	{
		if (line->x1 > line->x2)
			swap_points(line);
		drawline_low(line, mlx);
	}
	else
	{
		if (line->y1 > line->y2)
			swap_points(line);
		drawline_high(line, mlx);
	}
}
